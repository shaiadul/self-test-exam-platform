...
export const ExamInstructionsScreen: React.FC<ExamInstructionsScreenProps> = ({
  ...
  return (
    <PageContainer className="max-w-3xl mx-auto py-3 sm:py-10 px-2.5 sm:px-4">
      <div className="bg-white rounded-lg sm:rounded p-3.5 sm:p-8 border border-slate-200/80 shadow-xs space-y-4 sm:space-y-6">
        {/* Header */}
        <div className="border-b border-slate-100 pb-4 sm:pb-5 text-center space-y-1.5 sm:space-y-2">
          <span className="px-2.5 py-0.5 bg-primary/10 text-primary font-mono font-bold text-[10px] rounded uppercase tracking-wider border border-primary/20 inline-block">
            {examMeta.subject}
          </span>
          <h1 className="text-xl sm:text-3xl font-bold text-slate-900 tracking-tight leading-snug">
            {examMeta.title}
          </h1>
        ...
        {(examMeta.startDate || examMeta.endDate) && (
          <div className="p-3 sm:p-3.5 bg-slate-50 rounded-lg border border-slate-200/90 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2.5 sm:gap-3 text-xs font-mono">
            <div className="flex items-center gap-2 text-slate-700">
              <FaCalendarAlt className="text-primary text-sm shrink-0" />
              <span className="font-bold">Exam Schedule Window:</span>
            </div>
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-1.5 sm:gap-4 text-[11px] text-slate-600 w-full sm:w-auto">
              <div>
        ...
        {isUpcoming && (
          <div className="p-3.5 sm:p-4 bg-amber-50 border border-amber-200 rounded-lg text-center space-y-1">
            <p className="text-xs sm:text-sm font-bold text-amber-900 flex items-center justify-center gap-2">
        ...
        {isExpired && (
          <div className="p-3.5 sm:p-4 bg-rose-50 border border-rose-200 rounded-lg text-center space-y-1">
            <p className="text-xs sm:text-sm font-bold text-rose-900 flex items-center justify-center gap-2">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3 p-3 sm:p-4 bg-slate-50 border border-slate-200/80 rounded-lg text-center">
          <div className="p-1.5 sm:p-2">
            <p className="text-[10px] font-mono text-slate-400 uppercase">Questions</p>
            <p className="text-lg sm:text-xl font-mono font-black text-slate-900">
              {loadingQuestions ? "..." : questions.length}
            </p>
          </div>
          <div className="p-1.5 sm:p-2 border-l border-slate-200/80">
            <p className="text-[10px] font-mono text-slate-400 uppercase">Duration</p>
            <p className="text-lg sm:text-xl font-mono font-black text-primary">{examMeta.durationMinutes} Mins</p>
          </div>
          <div className="p-1.5 sm:p-2 border-t sm:border-t-0 sm:border-l border-slate-200/80">
            <p className="text-[10px] font-mono text-slate-400 uppercase">Total Marks</p>
            <p className="text-lg sm:text-xl font-mono font-black text-slate-900">{examMeta.totalMarks}</p>
          </div>
          <div className="p-1.5 sm:p-2 border-t sm:border-t-0 border-l border-slate-200/80">
            <p className="text-[10px] font-mono text-slate-400 uppercase">Negative Marks</p>
            <p className="text-lg sm:text-xl font-mono font-black text-rose-500">
        ...
        <div className="space-y-2.5 sm:space-y-3 bg-slate-50/70 border border-slate-200/80 rounded-lg p-3 sm:p-5">
          ...
          <ul className="text-xs text-slate-600 space-y-2 list-disc list-inside font-medium leading-relaxed">
            ...
            <li>
              <strong className="text-slate-900 font-bold">Question Palette:</strong> Use the Question Matrix to flag, jump, or review items anytime.
            </li>
          </ul>
        </div>
        ...
        {!loadingQuestions && questions.length === 0 && (
          <div className="p-3.5 sm:p-4 bg-amber-50 border border-amber-200 rounded-lg text-center space-y-2">
        ...
        {/* Actions */}
        <div className="pt-2 border-t border-slate-100 flex flex-col-reverse sm:flex-row gap-2.5 sm:gap-3">
          <OutlineBtn
            onClick={onExit}
            className="w-full sm:flex-1 !text-xs !py-3 sm:!py-2.5 !rounded-lg sm:!rounded justify-center"
          >
            Exit to Dashboard
          </OutlineBtn>
          <PrimaryBtn
            ...
            className="w-full sm:flex-1 !text-xs !py-3 sm:!py-2.5 !rounded-lg sm:!rounded shadow-xs gap-2 disabled:opacity-50 justify-center"
          >
