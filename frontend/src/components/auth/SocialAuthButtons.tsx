    ...
    <div className={`w-full space-y-2.5 ${className}`}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
        ...
        <button
          ...
          className="relative inline-flex items-center justify-center gap-2.5 px-4 py-2 bg-white hover:bg-slate-50/90 active:bg-slate-100 text-slate-700 hover:text-slate-900 text-xs sm:text-sm font-semibold rounded-lg border border-slate-200/90 shadow-2xs hover:shadow-xs transition-all duration-150 cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed group focus:outline-none focus-visible:ring-2 focus-visible:ring-[#24292f]/40"
          aria-label={`${actionLabel} with GitHub`}
        >
          {loadingProvider === "github" ? (
            <div className="w-4 h-4 border-2 border-slate-300 border-t-[#24292f] rounded-full animate-spin" />
          ) : (
            <svg
              className="w-4 h-4 shrink-0 transition-transform duration-150 group-hover:scale-105"
              viewBox="0 0 24 24"
